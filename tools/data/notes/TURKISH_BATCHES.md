...
| 2026-08-31 | `msgbatch_01PjmPDmm3SDuLyjoYDMo29a` | 621 | LABEL translation for zh_multitask_6k: 24,811 Chinese label keys -> English (~$0.55). Recovers a corpus annotated with Chinese labels without re-annotating it -- GLiNER2 takes the label set as an INPUT, so Chinese labels train a DISJOINT space where `person` returns nothing on a Chinese document | `uv run python tools/data/translate_labels.py --in data/chinese_gate/zh_multitask_6k.jsonl --out data/chinese_gate/zh_multitask_6k_en.jsonl --batch` |
