...
| 2 | **eb17-best: the warm-start base** | folds in every measured lesson; adds label negatives, which four dead mechanisms have waited on | **CRASHED 22:03 UTC 2026-09-21** at step 6,930/63,270 (epoch 0.50), no checkpoint; box terminated (~$6.85). `ValueError: record 'record' declares anchor 'type' ...` in a train-loader worker. **ROOT CAUSE OPEN** — data proven clean (0 broken anchors over 202,211 records; text2json byte-identical to HF) and the full mixture does NOT reproduce locally (203,736 chunks, 0 raises) | close the local/HF corpus divergence first (box loaded 201,074, local list yields 202,211 — **1,137 records I do not have**), then relaunch |
...
| 16 | **EVENT ARGUMENTS: does the binding objective we ALREADY have work?** | strict argument F1 requires the argument to attach to the right trigger. We DO train that -- `record_loss_weight` defaults to 1.0 and `compute_group_loss` seeds an instance from the ANCHOR, supervising fields into it. An earlier note here claimed we never train it; that was wrong and is retracted in EVENT_ARGUMENT_DIAGNOSIS 4c-i | **OPEN, and the headline figure does not measure the current architecture.** The 0.1178 strict / 0.5783 relaxed spread was measured WITHOUT `event_records`, on a record head that had never seen an event -- so it says nothing about what eb17 trains. OneIE (verified in code 2026-09-22) trains the same binding in a different FORM: pairwise classification over (trigger, entity) candidates with a null role, negative-saturated by construction, at sentence scope where the pair set is small | **1.** Add the Arg-C key (exact surface + type + role, NO trigger) to `eval_metrics.py` -- free, open since 2026-09-15, and the only way to compare to the literature. **2.** Read strict argument F1 off an `event_records: true` base -- eb17 was bought for this and has not delivered it. **3.** ONLY IF (2) is still at the floor, consider a pairwise role objective; note the pair set is quadratic over a 4096-token window, where OneIE's is sentence-sized |
| 13 | **Chunking silently drops `record_metadata`** | every CHUNKED long document loses record supervision — the structure becomes undecodable and trains NOTHING, with no error. Same shape as the already-priced +0.182 structure F1 repair | **PROVEN, live on the box** (`RuntimeWarning: Structure schema ['record'] carries no record_metadata` from step 39) and locally (7,754 → 7,656 of 8,303 chunks) | `chunking.py::_filter_record_output` must carry `record_metadata`; **test first** — restoring it makes the anchor a `REQUIRED_ONE` field whose surface may fall outside the window |
