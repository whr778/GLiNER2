...
<a id="todo-archive-2026-09-21"></a>

# Archive: the full TODO list as it stood on 2026-09-21

TODO.md was rewritten on 2026-09-21 to hold ONLY outstanding work, because it had grown to
1,331 lines in which resolved narratives outnumbered live items and the live ones could not be
found. **The entire prior contents are preserved verbatim below rather than summarised**, so
that anything mis-classified as historical during that rewrite is still recoverable. Sections
here are dated and many are superseded -- read TODO.md for what is actually open, and the
[[EXPERIMENT_CATALOG]] for what was run.
...
| **does a typed-role constraint help (option 2)?** | **ANSWERED -- NO, for the trained margin** | `typed_margin_mask` built 2026-09-21 and the ceiling MEASURED rather than argued. The mask fires (0.339% of candidate cells, 91 queries with a disallowed competitor) and moves nothing: the loss effect at delta=ln2 is **0.00% median, 0.28% mean**, with only 5 of 91 affected queries above w_S 0.01. The model already scores type-incompatible fillers near zero. A 0.28% mean gradient perturbation cannot move F1 against a +/-0.02 floor, so the ~$34-65 trained arm is CANCELLED. Decode-time remains a null on F1 with a real precision/recall trade (98% of drops were false positives); map reach is 99.3%. See `OPTION_2_TYPED_ROLE_CONSTRAINTS.md` |
