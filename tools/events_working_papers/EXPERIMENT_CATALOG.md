...
| 2026-09-20 | **EKF** | **option 4 roles A/B RE-RUN** (`roles2`, us-east-1) | does the derived corpus help once BOTH arms are regularised by label negatives? | 2× A100, ~16h | **RUNNING** — the first run had NO negative signal anywhere (injector unwired, derived corpus absent from the pools file, 0% of entity labels absent in the mix). Both arms now inject at **~4.9 absent queries/forward vs 0.37**; the derived corpus keeps `partial_annotation` on 42.3% evidence | ~$65 | `OPTION_4_ROLE_TO_ENTITY.md` |
