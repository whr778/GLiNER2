    ...
    def build(self) -> Dict[str, Any]:
        ...
        if self._events is not None:
            ...
            if isinstance(events, list):
                # Normalize the list form to the {type: config} dict the schema
                # parser expects.
                events = {
...
class GLiNER2API:
    ...
    def extract(
        ...
        if not any(key in schema_dict for key in [
            "entities", "classifications", "structures", "relations", "events"
        ]):
    ...
    def extract_events(
        ...
        """
        schema_dict = SchemaAPI().events(event_types).build()
        return self._task_call(
            "schema", text, schema_dict,
            threshold, format_results, include_confidence, include_spans,
        )
