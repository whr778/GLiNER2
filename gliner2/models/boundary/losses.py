...
def _reduce(
    ...
    """Reduce a masked ``[B, Q, N]`` loss globally, per active query, or by sum.
    ...
    analogue of the phase-2 span model's per-task event loss. It does NOT apply to
    ``mode="sum"``, which is a plain masked total.

    For the averaging modes the denominator stays **unweighted** on purpose. A
    weighted mean would make the loss invariant to scaling every weight, so raising
    the event weight would do nothing; dividing by the plain active count means a
    weight of 2.0 really does double that task's share of the gradient. At all-ones
    this reduces exactly to the unweighted form.
    """
